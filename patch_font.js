...
const targetStr = `        var qTxt = AddText(panelGo.transform, "Question", $"<i>{promptTitle}</i>", 26, ThemeUI.TextMuted, FontStyle.Normal, TextAnchor.MiddleCenter);
...
const replacement = `        var qTxt = AddText(panelGo.transform, "Question", $"<i>{promptTitle}</i>", 30, ThemeUI.TextMuted, FontStyle.Normal, TextAnchor.MiddleCenter);
        SetRect(qTxt.rectTransform, new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(850f, 220f), new Vector2(0, -100f)); 
        qTxt.horizontalOverflow = HorizontalWrapMode.Wrap; 
        qTxt.verticalOverflow = VerticalWrapMode.Overflow;
...
if (nCode.includes(nTarget)) {
    ...
    console.log('SUCCESS: Nullify font fix');
} else {
    console.log('FAILED TO FIND Nullify font string');
}
