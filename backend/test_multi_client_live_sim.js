...
const players = [
  { userId: "ai_bot",                  generalName: "Quang Trung [AI]", maxHp: 4, isAI: true  },
...
// ═══════════════════════════════════════════
// LƯỢT 1 — QUANG TRUNG [AI] (G1) 🔵
// ═══════════════════════════════════════════
console.log("\n\n══ LƯỢT 1 — Quang Trung [AI] (G1) ══");
p1.hand = [WINE("w1"), SLASH("s1"), DODGE("d1"), PEACH("pc1")];
p2.hand = [DODGE("d2"), SLASH("s2"), C("ar2", "Mưa Tên Liên Châu", "Heart", 1, 2, CARD_SUBTYPES.ARROW_RAIN), EX("ex1")];
p3.hand = [DODGE("d3"), DODGE("d3b"), PEACH("pc3"), DUEL("duel3")];
p4.hand = [SLASH("s4"), PEACH("pc4"), EX("ex4")];

step("G1 (AI) tự động xử lý lượt");
handleAIStep(state);
log(state);
if (state.phase === "PLAY" && state.turnSeat === 1) {
  step("G1 (AI) kết thúc lượt 1");
...
p2.hand = [C("ar2", "Mưa Tên Liên Châu", "Heart", 1, 2, CARD_SUBTYPES.ARROW_RAIN), SLASH("s2b"), EX("ex2")];
...
while (state.phase === "AWAIT_AOE") {
  ...
  const hasDodge = wp.hand.some(c => c.subType === CARD_SUBTYPES.DODGE);

  if (hasDodge) {
    const dCard = wp.hand.find(c => c.subType === CARD_SUBTYPES.DODGE);
    step(`G${who} (${wp.generalName}) có lá [Đỡ] -> Dùng [Đỡ] để né Mưa Tên!`);
    handleRespondAction(state, who, true, dCard.id);
  } else {
    step(`G${who} (${wp.generalName}) không có [Đỡ] -> Chịu 1 sát thương`);
    handleRespondAction(state, who, false, null);
  }
  log(state);
}

assert(state.phase === "PLAY", "Sau Mưa Tên, Phase phải trở về PLAY");
console.log("       ✅ Đã giải quyết xong MƯA TÊN LIÊN CHÂU hoàn toàn trơn tru!");

// ─── G2 ĐÁNH TIẾP DỤNG BINH NHƯ THẦN VÀ KẾT THÚC LƯỢT ───
